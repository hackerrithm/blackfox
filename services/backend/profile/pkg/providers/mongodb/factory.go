...
var (
	profileRepositoryInstance engine.ProfileRepository
	profileRepositoryOnce     sync.Once
)
...
// NewProfileRepository creates a new datastore Profile repository
func (f *storageFactory) NewProfileRepository() engine.ProfileRepository {
	profileRepositoryOnce.Do(func() {
		profileRepositoryInstance = newProfileRepository(f.session)
	})
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(profileCollection)
	c.EnsureIndex(index)
}
