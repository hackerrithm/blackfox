...
var (
	groupRepositoryInstance engine.GroupRepository
	groupRepositoryOnce     sync.Once
)
...
// NewGroupRepository creates a new datastore Group repository
func (f *storageFactory) NewGroupRepository() engine.GroupRepository {
	groupRepositoryOnce.Do(func() {
		groupRepositoryInstance = newGroupRepository(f.session)
	})
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(groupCollection)
	c.EnsureIndex(index)
}
