...
var (
	orderRepositoryInstance engine.OrderRepository
...
// NewOrderRepository creates a new datastore Order repository
func (f *storageFactory) NewOrderRepository() engine.OrderRepository {
	orderRepositoryOnce.Do(func() {
		orderRepositoryInstance = newOrderRepository(f.session)
	})
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(orderCollection)
	c.EnsureIndex(index)
}
