...
var (
	matchRepositoryInstance engine.MatchRepository
	matchRepositoryOnce     sync.Once
)
...
// NewMatchRepository creates a new datastore Match repository
func (f *storageFactory) NewMatchRepository() engine.MatchRepository {
	matchRepositoryOnce.Do(func() {
		matchRepositoryInstance = newMatchRepository(f.session)
	})
	return matchRepositoryInstance
}
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(matchCollection)
	c.EnsureIndex(index)
}
