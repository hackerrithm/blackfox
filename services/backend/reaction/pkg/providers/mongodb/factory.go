...
var (
	reactionRepositoryInstance engine.ReactionRepository
	reactionRepositoryOnce     sync.Once
)
...
// NewReactionRepository creates a new datastore Reaction repository
func (f *storageFactory) NewReactionRepository() engine.ReactionRepository {
	reactionRepositoryOnce.Do(func() {
		reactionRepositoryInstance = newReactionRepository(f.session)
	})
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(reactionCollection)
	c.EnsureIndex(index)
}
