...
var (
	walletRepositoryInstance engine.WalletRepository
	walletRepositoryOnce     sync.Once
)
...
// NewWalletRepository creates a new datastore Wallet repository
func (f *storageFactory) NewWalletRepository() engine.WalletRepository {
	walletRepositoryOnce.Do(func() {
		walletRepositoryInstance = newWalletRepository(f.session)
	})
...
func ensureIndexes(s *mgo.Session) {
	...
	c := s.DB("test1").C(walletCollection)
	c.EnsureIndex(index)
}
