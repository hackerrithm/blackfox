//go:generate protoc ./chat.proto --go_out=plugins=grpc:./pb
...
// ListenGRPC ...
func ListenGRPC(s engine.Chat, userURL string, port int) error {
	...
	serv := grpc.NewServer()
	pb.RegisterChatServiceServer(serv, &grpcServer{s, profileClient})
...
func (s *grpcServer) Post(ctx context.Context, r *pb.PostRequest) (*pb.PostResponse, error) {
	...
	if err != nil {
		log.Println("here error in chat method")
		return nil, err
	}
...
func (s *grpcServer) GetMultiple(ctx context.Context, r *pb.GetMultipleRequest) (*pb.GetMultipleResponse, error) {
	...
	res, err := s.service.ListAllChats(ctx, r.Skip, r.Take)
	if err != nil {
		return nil, err
	}
	chats := []*pb.Chat{}
